...
// 给其他节点发送信息
func (er *EdgeRelay) MsgToOtherEdge(addr string, container *mux.MessageContainer) {
	// 启动client，发送信息
}
...
func (er *EdgeRelay) GetAddress(nodeID string) string {
	return ""
}

// 从文件中读取所有的nodeID：addr键值对，返回一个map
func (er *EdgeRelay) GetAllAddress() map[string]string {
	return nil
}
